...
def main():
    ...
    for i in range(len(frames)-1):
        ...
        spatial_features = function.extract_spatial_features(frames[i])
        temporal_features = function.extract_temporal_features(frames[i+1], frames[i])
        motion_map = np.random.rand(frames[i].shape[0], frames[i].shape[1])
        fused_features = function.motion_based_feature_fusion(spatial_features, temporal_features, motion_map)
        total_features.append(fused_features)


    # cv2.destroyAllWindows()
    video_quality_score = function.spatial_temporal_feature_pooling(np.array(total_features))
    print("Video Quality Score:", video_quality_score / len(frames - 1))
